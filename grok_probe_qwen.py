...
VISION_DIM = 4096           # MoonViT merged-token width (2x2 patch-merge flatten)
...
WARMUP_STEPS = 100
GRAD_CLIP = 1.0
MAX_SEQ_LEN = 4096          # same text budget as modal_train.py
EPOCHS = 2
...
CHART_EVERY = 50            # re-render probe_curves.png
SAVE_EVERY = 500            # resumable checkpoint cadence
STATUS_EVERY = 20           # human-readable console line

# Grok reference: Baseten GLM-5.2 recipe collapsed at ~step 900 @ batch 64 /
# 66k images == ~57.6k samples seen.
SAMPLES_PER_BASETEN_GROK = int(900 * 64)
PLATEAU_CHECK_EVERY = 50    # steps between flat-loss banner evaluations
PLATEAU_WINDOW = 300        # steps of loss history considered "flat"
PLATEAU_REL_TOL = 0.02      # |ema - old_mean| / old_mean below this => flat
EMA_BETA = 0.98
SPIKE_FACTOR = 2.0          # alert when EMA exceeds 2x its recent floor (see ProbeMonitor)
SPIKE_WINDOW = 100          # EMA history window defining that floor
SPIKE_MIN_HISTORY = 20      # don't alert before the EMA stream has settled
