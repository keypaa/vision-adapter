...
import modal
from vision_adapter.config import default_config as _default_config, config_header as _config_header
...
# scattered BATCH_SIZE/LR/MAX_SEQ_LEN/EPOCHS at modal_train:49 /
# modal_probe:65 / grok_probe:104). Module-level aliases kept for
# test + intra-file references that haven't migrated to cfg yet.
_CFG_DEFAULT = _default_config()
BATCH_SIZE = _CFG_DEFAULT.batch_size
LR = _CFG_DEFAULT.lr
MAX_SEQ_LEN = _CFG_DEFAULT.max_seq_len
EPOCHS = _CFG_DEFAULT.epochs
SAMPLES_PER_BASETEN_GROK = _CFG_DEFAULT.samples_per_baseten_grok
LOG_EVERY = _CFG_DEFAULT.log_every
VAL_EVERY = _CFG_DEFAULT.val_every
SAVE_EVERY = _CFG_DEFAULT.save_every
...
# vision_adapter package for the unified config (local import must be on the image)
train_image = train_image.add_local_dir("vision_adapter", "/root/vision_adapter")
train_image_b300 = train_image_b300.add_local_dir("vision_adapter", "/root/vision_adapter")
